...
grad_clip = 50  # clip if gradients are exploding, which may happen at larger batch sizes (sometimes at 32) - you will recognize it by a sorting error in the MuliBox loss calculation
...
#                                     lr=lr, momentum=momentum, weight_decay=weight_decay)
        optimizer = torch.optim.Adamax(params=[{'params': biases, 'lr': 2 * lr}, {'params': not_biases}],
...
#             print(losses.val,losses.avg)
            ...
            f = open('sgd_bz_adamax_16_train.txt','a')
            arr = list([epoch,i,losses.val])
            f.write(str(arr)+'\n')
            f.close()
            ...
            if i % print_freq == 0:
                print('[{0}/{1}]\t'
                      ...
                      'Loss {loss.val:.4f} ({loss.avg:.4f})\t'.format(i, len(val_loader),
                                                                      batch_time=batch_time,
                                                                      loss=losses))
                f = open('sgd_bz16_adamax_val.txt','a')
                arr = list([i,losses.val])
                f.write(str(arr)+'\n')
                f.close()
